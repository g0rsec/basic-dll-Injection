    ...
    unsafe {
        match get_process_entry(PROCESS_NAME) {
            Ok(process_entry) => {
                ...
                let dll_path =
                    CString::new(dll_file_path.canonicalize().unwrap().to_str().unwrap()).unwrap();
                ...
                let mod_handle = GetModuleHandleA(PSTR {
                    0: kernel_mod.as_ptr() as *mut u8,
                });

                let mod_address = GetProcAddress(
                    mod_handle,
                    PSTR {
                        0: loadlibrary_fn.as_ptr() as *mut u8,
                    },
                )
                .unwrap();
